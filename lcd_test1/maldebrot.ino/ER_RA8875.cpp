...
// ER_RA8875.cpp  (dentro de ER_RA8875::begin)

pinMode(_cs, OUTPUT);
digitalWrite(_cs, HIGH);

pinMode(_rst, OUTPUT);
// HARD reset real (como tu low-level que sí funcionó)
...
// En ESP32, inicializa SPI primero y con pines explícitos
#if defined(ESP32)
  SPI.begin(18, 19, 23, _cs); // SCK=18, MISO=19, MOSI=23, SS/CS=_cs
#else
  SPI.begin();
#endif

SPI.beginTransaction(SPISettings(1000000, MSBFIRST, SPI_MODE0)); // arranca a 1MHz para bring-up
...
// Ahora sí puedes subir velocidad si quieres (opcional)
SPI.endTransaction();
SPI.beginTransaction(SPISettings(4000000, MSBFIRST, SPI_MODE0)); // se pueden 20 mhz pero para protoboard solo usaremos 4Mhz
...
boolean ER_RA8875::waitPoll(uint8_t regname, uint8_t waitflag) {
  const uint32_t timeoutMs = 500;   // 500–2000ms es razonable para pruebas
  uint32_t t0 = millis();

  while ((millis() - t0) < timeoutMs) {
    uint8_t temp = readReg(regname);
    if (!(temp & waitflag)) return true;
    yield();                        // clave en ESP32 (evita WDT)
  }

  // Si quieres, aquí puedes imprimir debug (ver nota abajo)
